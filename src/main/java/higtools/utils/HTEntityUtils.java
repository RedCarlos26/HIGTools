...
public class HTEntityUtils {
    ...
    public static BlockPos getCityBlock(PlayerEntity player) {
        ...
        posList.sort(Comparator.comparingDouble(HTPlayerUtils::distanceFromEye));
    ...
    public static BlockPos playerPos(PlayerEntity targetEntity) {
        return HTWorldUtils.roundBlockPos(targetEntity.getPos());
    }
    ...
    public static boolean isBurrowed(PlayerEntity targetEntity, BlastResistantType type) {
        BlockPos playerPos = HTWorldUtils.roundBlockPos(new Vec3d(targetEntity.getX(), targetEntity.getY() + 0.4, targetEntity.getZ()));
