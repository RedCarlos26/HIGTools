...
public class HighwayBuilderPlus extends Module {
    ...
    public HighwayBuilderPlus() {
        super(HighwayTools.MAIN, "highway-builder+", "Automatically builds highways.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        if (width.get() < 3 && dir.diagonal) {
            ...
            if (Modules.get().get(highwaytools.modules.main.HighwayTools.class).isActive()) {
                Modules.get().get(highwaytools.modules.main.HighwayTools.class).toggle();
            }
            return;
        }
        ...
        if (Modules.get().get(highwaytools.modules.kmain.AutoEatPlus.class).getEating()) return;
